"""
leetcode solution for problem no 2300.
"""
...
class Solution: #pylint: disable=too-few-public-methods
    ...
    def successfulPairs(self, spells: List[int], potions: List[int], success: int) -> List[int]:
        ...
        for _, value in enumerate(spells):
            successful_pairs: int = 0
            left_index: int = 0
            right_index: int = len(potions) - 1
            while left_index <= right_index:
                mid_index: int = left_index + (right_index - left_index) // 2
                if value * potions[mid_index] >= success:
                    right_index = mid_index - 1
                else:
                    left_index = mid_index + 1

            successful_pairs = len(potions) - left_index
